...
class Image
{
	...
	/**
	 * 对指定的图像进行缩放
	 * @param	string	$filename 是需要处理的图片名称路径
	 * @param	int	$width	缩放后的宽度
	 * @param	int	$height	缩放后的高度
	 * @param	string	$qz	是新图片的前缀
	 * @return	mixed		是缩放后的图片名称,失败返回false;
	 */
	public function thumb($filename, $width, $height, $newimgname)
	{
		$imgInfo=$this->getInfo($filename); //获取图片信息
		$srcImg=$this->getImg($filename, $imgInfo); //获取图片资源         
		$size=$this->getNewSize($filename,$width, $height,$imgInfo);//获取新图片尺寸
		$newImg=$this->kidOfImage($srcImg, $size,$imgInfo);   //获取新的图片资源
		return $this->createNewImage($newImg, $newimgname,$imgInfo);    //返回新生成的缩略图的名称，以"th_"为前缀
	}
	...
	*/ 
	public function waterMark($groundName, $waterName, $waterPos=0, $qz="wa_")
	{
		...
		if(file_exists($groundName) && file_exists($this->pathwater.$waterName)){
	...
	/**
	 * 图片旋转
	 * @param $file array file域信息
	 * */
	public function newexif($file)
	{
		$exif = @exif_read_data($file);
		$exif_orient = isset($exif['Orientation']) ? $exif['Orientation']:0;
		$MimeType = isset($exif['MimeType']) ? $exif['MimeType'] : '';
		$rotateImage = 0;
	 ...
	 * 创建图像资源
	 * */ 
	private function getImg($filename, $imgInfo)
	{
		//$this->path=rtrim($this->path,"/")."/";
		$srcPic= $filename;
